...
ThreadPool::ThreadPool() : MaxThreadNum(5), RunThreadNum(0), stop(false)
{
	InitThreadPool();
}

ThreadPool::ThreadPool(int size) : MaxThreadNum(size), RunThreadNum(0), stop(false)
{
	InitThreadPool();
}
...
void ThreadPool::InitThreadPool()
{
	...
	{
		HANDLE hd = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)Worker, (LPVOID*)this, 0, NULL); //Î´¹ÒÆð
		Handles.PushBack(&hd);
	}
}
